...
// When multiple second-factor methods are enrolled IBKR shows a picker.
// The Authenticator-App entry is what we want; selectors here match
// the labels and buttons we've seen on that picker.
const PICKER_AUTHAPP_SELECTORS = [
  'text=/authenticator app/i',
  'text=/soft token/i',
  'text=/^totp$/i',
  '[data-method*="authenticator"]',
  '[data-method*="softtoken"]',
];
...
async function handleTotp(page, totpSecret, onProgress, timeoutMs) {
  ...
  let field = null;
  while (Date.now() < deadline) {
    picker = await findFirstVisible(page, PICKER_AUTHAPP_SELECTORS);
  ...
  if (picker) {
    ...
    // Wait for the code field to appear after the click.
