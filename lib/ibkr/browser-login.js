...
export async function loginWithBrowser({
  ...
  try {
    ...
    // New-device / email-verification stage. IBKR's "Login Security
    // Code" prompt — they emailed a 6-digit "Temporary Security Code"
    // because the headless Chromium looks like an unrecognised device.
    // The input lives in xyzblock-temp (#xyz-field-temp-response).
    // If we have an emailCode in hand, fill it. Otherwise throw a
    // dedicated error so the caller (gateway) can surface a UI prompt
    // and retry with the code once the user reads their email.
