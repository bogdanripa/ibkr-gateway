...
async function handleRestrictedVerification(page, emailCode, onProgress) {
  ...
  // Wait for either an explicit rejection (.v-email-failed becomes
  // visible — IBKR removes its display-none class on bad token) or
  // the page leaves /restricted/. After a successful verify the page
  // may show a "Continue login" button — click it if present.
  const retryDeadline = Date.now() + 20_000;
  while (Date.now() < retryDeadline) {
    // Precise selector: the .v-email-failed <p> element. Default
    // state has class "display-none"; on rejection IBKR adds
    // inline style="display: block" so the element renders.
    const rejected = await page
      ...
      .catch(() => false);
    if (rejected) {
      // Treat as "needs a fresh code" rather than a hard credential
      // failure — the credentials ARE valid, only the email token is
      // wrong/expired. The caller's UI re-prompts.
      const err = new EmailVerificationRequiredError();
      err.previousRejected = true;
      throw err;
    }
