...
export async function loginWithBrowser({
  ...
  emailCode = null,
  /**
   * Async callback the driver invokes when IBKR's new-device
   * `/restricted/` page is shown and we need an email verification
   * code. Receives `{ previousRejected: boolean }`. Must resolve to
   * a code string (continue) or null (abort, throw
   * EmailVerificationRequiredError).
   *
   * If omitted, the driver throws EmailVerificationRequiredError the
   * first time the page is hit (legacy single-shot mode). When
   * supplied, the BROWSER STAYS OPEN across as many code attempts as
   * the user wants — critical because IBKR binds the email token to
   * the specific browser session that triggered it.
   */
  onNeedCode = null,
  ...
  try {
    ...
    await handleRestrictedVerification(page, emailCode, onNeedCode, onProgress);
 ...
 */
async function handleRestrictedVerification(page, initialCode, onNeedCode, onProgress) {
  ...
  // Loop: ask for code → fill → submit → wait for transition or
  // explicit rejection. On rejection ask the caller for ANOTHER code
  // (same browser, no re-auth — critical because IBKR only honours
  // the token for the session it generated it for).
  let previousRejected = false;
  let attempt = 0;
  while (attempt++ < 8) {
    let code = attempt === 1 ? initialCode : null;
    if (!code) {
      // Need the caller to supply one. Either the caller passes a
      // resumable callback (preferred — keeps the browser alive) or
      // we throw the single-shot error (legacy compat).
      if (!onNeedCode) {
        const err = new EmailVerificationRequiredError();
        err.previousRejected = previousRejected;
        throw err;
      }
      onProgress(previousRejected
        ? 'waiting for a fresh code (previous rejected)'
        : 'waiting for verification code');
      code = await onNeedCode({ previousRejected });
      if (!code) {
        // Caller aborted.
        throw new EmailVerificationRequiredError();
      }
    ...
    // Clear any previous value before refilling.
    await input.fill('');
    await input.fill(String(code).trim());

    const submit = page.locator('form.form-email button[type="submit"].btn-primary').first();
    await submit.click({ timeout: 5_000 });
    onProgress(`submitted verification token (attempt ${attempt})`);

    // Wait for transition OR explicit rejection.
    const retryDeadline = Date.now() + 20_000;
    let rejectedThisRound = false;
    let succeeded = false;
    while (Date.now() < retryDeadline) {
      const rejected = await page
        .locator('.v-email-failed')
        .first()
        .isVisible({ timeout: 100 })
        .catch(() => false);
      if (rejected) {
        rejectedThisRound = true;
        // Hide the .v-email-failed indicator so a subsequent rejection
        // doesn't trigger immediately on a stale match before IBKR
        // re-shows it. We toggle the inline style.
        await page.evaluate(() => {
          for (const el of document.querySelectorAll('.v-email-failed')) {
    ...
    if (rejectedThisRound) {
      ...
      continue; // loop back; ask caller for another code
    }
    // Neither rejected nor transitioned within retryDeadline →
    // something else is wrong (maybe IBKR also needs phone/AU10TIX).
    throw new Error(
  ...
  throw new Error('Too many failed verification attempts — give up and request a fresh email.');
}
