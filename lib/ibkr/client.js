...
import { assertValidTotpSecret } from './totp.js';
...
export class IbkrClient {
  ...
  // headed:      open visible Chromium (useful for IBKey push / debugging)
  // totpSecret:  base32 activation code for IBKR's Authenticator App.
  //              REQUIRED for unattended live re-auth (without it the
  //              gateway can't bring a dropped session back up). Paper
  //              accounts cannot have 2FA so this is ignored when
  //              mode='paper'.
  // onProgress:  (msg: string) => void
  async signIn({
    username, password, mode = 'live',
    headed = false, totpSecret = null,
    emailCode = null,
    onNeedCode = null,
    onProgress = () => {}, debugDir,
  } = {}) {
    ...
    if (!paper && !totpSecret) {
      throw new IbkrError(
        'signIn: live mode requires totpSecret (the Authenticator App activation code). ' +
        'See https://ibkr-gateway.bogdanripa.com/help/authenticator-app',
        { stage: 'totp-required' },
      );
    }
    if (totpSecret) {
      // Reject malformed secrets up-front, before we spawn Chromium.
      assertValidTotpSecret(totpSecret);
    }
    const { cookies, apiBase, portalPrefix, finalUrl } = await loginWithBrowser({
      username, password, paper,
      totpSecret: paper ? null : totpSecret,
      emailCode,
      onNeedCode,
      // Carry every cookie we've previously captured into the new
      // browser context so IBKR sees us as a returning device and
      // is less likely to re-trigger the new-device challenge.
      priorCookies: this.state.cookies,
      headed, onProgress, debugDir,
    });
  ...
  async _validateSso() {
    // Live sign-ins occasionally race the USERID cookie — the browser
    // captured XYZAB and we extracted the jar before USERID landed.
    // Retry the call once after a brief delay if IBKR specifically
    // complains about the missing cookie.
    let res = await this._get(this._portalUrl('sso/validate?gw=1'));
    if (res.status === 401 && /no userid cookie/i.test(JSON.stringify(res.data ?? ''))) {
      await SLEEP_MS(2000);
      res = await this._get(this._portalUrl('sso/validate?gw=1'));
    }
    const data = this._expect200('sso/validate', res);
  ...
  async searchSecurity(query, { name = false } = {}) {
    ...
    // IBKR's secdef/search endpoint regularly returns transient 5xx
    // (especially right after sign-in while the brokerage tier is
    // still warming up). Retry on 5xx; surface the last status if all
    // attempts fail.
    let res;
    for (let attempt = 0; attempt < 4; attempt++) {
      res = await this._get(this._portalUrl(`iserver/secdef/search?${qs}`));
      if (res.status === 200) break;
      if (res.status < 500) break;            // 4xx → no point retrying
      await SLEEP_MS(400 * (attempt + 1));    // 400, 800, 1200 ms backoff
    }
    if (res.status !== 200) {
      throw new IbkrError(`secdef/search ${query} → HTTP ${res.status} (after retries)`,
    ...
    return list
      .filter(Boolean)
      .map((r) => {
        const sections = Array.isArray(r.sections) ? r.sections.filter(Boolean) : [];
        // Currency: prefer the row-level field, fall back to the
        // first non-empty section currency (paper accounts often only
        // populate it on the section).
        const currency = r.currency
          || sections.find((s) => s && s.currency)?.currency
          || null;
        return {
          conid: Number(r.conid),
          symbol: r.symbol || null,
          description: r.companyHeader || r.description || r.companyName || r.symbol || null,
          currency,
  ...
  // Full security info by conid. Includes currency, listing exchange,
  // company name, valid exchanges, etc. (the /iserver/secdef/search
  // endpoint deliberately omits these to keep the symbol list small.)
  async getSecurityInfo(conid) {
    if (!conid) throw new IbkrError('getSecurityInfo: conid required');
    const res = await this._get(this._portalUrl(`iserver/secdef/info?conid=${encodeURIComponent(conid)}`));
    if (res.status !== 200) {
      throw new IbkrError(`secdef/info → HTTP ${res.status}`,
        { stage: 'secdef/info', status: res.status, body: res.data });
    }
    const d = res.data || {};
    return {
      conid: Number(d.conid),
  ...
  // (per https://www.interactivebrokers.com/campus/ibkr-api-page/cpapi-v1/#md-fields):
  //   31  Last price            70   Day high
  //   55  Symbol                71   Day low
  //   82  Change ($)            87   Volume
  //   83  Change (%)            7051 Company name
  //   84  Bid                   7295 Open today
  //   85  Ask size              7296 Close yesterday
  //   86  Ask                   6509 Service code / market data state
  //   88  Bid size
  //
  // NB: we previously used 7741 / 7762 / 7287 / 7288 here for
  // change / change% / dayHigh / dayLow — those are actually
  // position / mark-price / avgVolume(90) / avgVolume(30) in IBKR's
  // field map. The wrong mapping showed up in MCP as "huge changePct
  // numbers" + bogus dayHigh/dayLow.
  //
  // IBKR sometimes returns prefixed strings like "C24.50" (closing)
  // or "H123.4" (halted) for numeric fields. parseFloat handles the
  // leading sign + digits but balks on a leading letter; strip it.
  async getQuote(conid, {
    fields = [31, 55, 82, 83, 84, 85, 86, 87, 88, 70, 71, 7051, 7295, 7296, 6509],
    retries = 3,
  } = {}) {
    ...
    const num = (v) => {
      ...
      // IBKR returns numbers with:
      //   · leading status letters: "C" (closing) / "H" (halted) / "D" (delayed)
      //   · trailing % sign for change fields
      //   · comma thousand-separators on sizes ("1,600")
    ...
    return {
      ...
      bidSize: num(last['88']),
      askSize: num(last['85']),
      changeAbs: num(last['82']),
      changePct: num(last['83']),
      volume: last['87'] ?? null,
      dayHigh: num(last['70']),
      dayLow: num(last['71']),
      openToday: num(last['7295']),
      closeYesterday: num(last['7296']),
      // 6509 is a state string ("D" delayed, "R" realtime, etc.) —
      // surface it so consumers know whether the snapshot is live.
      marketDataState: last['6509'] ?? null,
      raw: last,
    };
  }
